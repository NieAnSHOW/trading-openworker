...
import type { PriceBar, TradeMarker, IndicatorPoint } from "../../api";
import {
  calcMA,
  calcBOLL,
  calcMACD,
...
const RANGE_BARS: Record<CandlestickRange, number> = {
  "1M": 22,
  "3M": 63,
  "6M": 126,
  "1Y": 252,
...
export function CandlestickChart({
  ...
  const baseData = useMemo(() => {
    ...
    const candle = data.map((d) => [Number(d.open), Number(d.close), Number(d.low), Number(d.high)]);
  ...
  const extraIndicators = useMemo(() => {
    ...
    return Object.entries(indicators).map(([name, points]) => {
      ...
      return {
        ...
        values: baseData.dates.map((d) => lookup.get(d) ?? null),
      };
    });
  ...
  useEffect(() => {
    ...
    for (const [key, { name, data: lineData }] of Object.entries(overlayMap)) {
      if (overlays.has(key as Overlay)) {
        overlaySeries.push({
          ...
          yAxisIndex: 0,
          symbol: "none",
          lineStyle: { color: OVERLAY_COLORS[colorIdx], width: 1 },
        });
      ...
      overlaySeries.push(
        {
          ...
          yAxisIndex: 0,
          symbol: "none",
          lineStyle: { color: t.bollColor, width: 0.8, type: "dashed" },
        },
        {
          name: "BOLL",
          type: "line",
          ...
          yAxisIndex: 0,
          symbol: "none",
          lineStyle: { color: t.bollColor, width: 1 },
        },
        {
          name: "BOLL-",
          type: "line",
    ...
    if (sub === "vol") {
      subSeries = [
        { name: "Vol", type: "bar", data: vol, xAxisIndex: 1, yAxisIndex: 1 },
      ];
      subYAxis = {
        ...subYAxis,
        axisLabel: {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          ...(subYAxis.axisLabel as Record<string, unknown>),
          formatter: (v: number) => abbreviateNum(v),
        },
      };
      ...
      subSeries = [
        {
          ...
          yAxisIndex: 1,
          symbol: "none",
          lineStyle: { width: 1, color: t.infoColor },
        },
        {
          name: "DEA",
          type: "line",
          ...
          yAxisIndex: 1,
          symbol: "none",
          lineStyle: { width: 1, color: t.warningColor },
        },
        {
          name: "MACD",
          type: "bar",
          data: m.histogram.map((v) => ({
            value: v ?? 0,
            itemStyle: { color: (v ?? 0) >= 0 ? t.upColor : t.downColor },
          })),
          xAxisIndex: 1,
          yAxisIndex: 1,
        },
    ...
    const extraSeries = extraIndicators.map((ind, i) => {
      ...
      return {
        ...
        lineStyle: {
          width: 1,
          color: OVERLAY_COLORS[(colorIdx + i) % OVERLAY_COLORS.length],
    ...
    const defaultStart =
      maxBars >= data.length
        ? 0
        : Math.max(0, 100 - (maxBars / data.length) * 100);
  ...
  return (
    <div className={cn(fill && "flex h-full min-h-0 flex-col")}>
      <div className="flex items-center gap-2 mb-1 flex-wrap">
        ...
        <div className="w-px h-3 bg-line/60" />

        {/* Indicator dropdown */}
        <div className="relative">
          <button
            onClick={() => setShowMenu(!showMenu)}
            className="flex items-center gap-1 px-2 py-0.5 rounded text-[10px] text-muted hover:text-ink hover:bg-chromeHover transition-colors"
          >
            Indicators ({overlays.size}) <Icon name="chevronDown" size={12} />
          </button>
          {showMenu && (
            <div
              className="absolute top-full left-0 mt-1 z-50 bg-panel border border-line rounded-lg shadow-lg p-2 min-w-[160px]"
              onMouseLeave={() => setShowMenu(false)}
            >
              {["MA", "Channel"].map((group) => (
                <div key={group}>
                  <p className="text-[9px] text-faint uppercase tracking-wider px-1 pt-1">
                    {group}
                  </p>
                  {OVERLAY_OPTIONS.filter((o) => o.group === group).map((o) => (
                    <label
                      key={o.id}
                      className="flex items-center gap-2 px-1 py-0.5 rounded hover:bg-chromeHover cursor-pointer"
                    >
                      <input
                        type="checkbox"
              ...
              <div className="border-t border-line mt-1 pt-1">
                <button
                  ...
                  className="text-[10px] text-muted hover:text-ink px-1 py-0.5 w-full text-left rounded hover:bg-chromeHover"
                >
                  Bare K (clear all)
                </button>
              </div>
